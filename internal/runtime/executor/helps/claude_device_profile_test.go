...
func TestDefaultClaudeDeviceProfileMeetsOpus55MinimumVersion(t *testing.T) {
	minimum := claudeCLIVersion{major: 2, minor: 1, patch: 280}
	got, ok := parseClaudeCLIVersion(defaultClaudeDeviceProfile(nil).UserAgent)
	if !ok {
		t.Fatalf("default Claude User-Agent is not parseable: %q", defaultClaudeDeviceProfile(nil).UserAgent)
	}
	if got.Compare(minimum) < 0 {
		t.Fatalf("default Claude Code version = %d.%d.%d, want at least 2.1.280", got.major, got.minor, got.patch)
	}
}
