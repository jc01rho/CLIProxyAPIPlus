...
func TestClaudeExecutor_NonClaudeRequestUsesPinnedVersionWithMessageFingerprint(t *testing.T) {
	...
	assertClaudeFingerprint(t, seenHeaders, "claude-cli/2.1.177 (external, cli)", "0.94.0", "v24.3.0", "MacOS", "arm64")
	...
	if got := system[0].Get("text").String(); got != "x-anthropic-billing-header: cc_version=2.1.177.e2d; cc_entrypoint=cli;" {
		t.Fatalf("billing header = %q, want pinned version with message fingerprint e2d", got)
	}
...
func TestClaudeExecutor_ConfirmedClaudeCodeRequestPreservesInteractiveIdentity(t *testing.T) {
	...
	assertClaudeFingerprint(t, seenHeaders, "claude-cli/2.1.220 (external, cli)", "0.94.0", "v24.3.0", "MacOS", "arm64")
...
func TestClaudeExecutor_CopiedVSCodeAgentSDKHeadersWithoutMetadataAreCloaked(t *testing.T) {
	...
	if got := seenHeaders.Get("User-Agent"); got != "claude-cli/2.1.177 (external, cli)" {
...
func TestClaudeExecutor_ExecuteOAuthCustomToolMCPAliasRoundTrip(t *testing.T) {
	...
	if got := upstreamHeaders.Get("User-Agent"); got != "claude-cli/2.1.177 (external, cli)" {
