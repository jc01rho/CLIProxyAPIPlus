...
func TestReverseRemapPassesThroughCallerMCPToolsOnVirtualServerCollision(t *testing.T) {
	...
	for _, name := range native {
		...
		restored, err := restoreClaudeOAuthToolNamesFromResponse(response, "", true, reverseMap)
		if err != nil {
		...
		restoredLine, errLine := restoreClaudeOAuthToolNamesFromStreamLine(line, "", true, reverseMap)
		if errLine != nil {
...
func TestReverseRemapOAuthToolNamesRestoresHybridPassthroughMCPTools(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			restored, err := restoreClaudeOAuthToolNamesFromResponse(resp, "", true, reverseMap)
			if err != nil {
			...
			restoredLine, errStream := restoreClaudeOAuthToolNamesFromStreamLine(line, "", true, reverseMap)
			if errStream != nil {
...
func TestReverseRemapOAuthToolNamesHybridPassthroughPrecedenceAndAmbiguity(t *testing.T) {
	...
	t.Run("ambiguous passthrough tool rejected", func(t *testing.T) {
		...
		_, err := restoreClaudeOAuthToolNamesFromResponse(resp, "", true, reverseMap)
		if err == nil {
