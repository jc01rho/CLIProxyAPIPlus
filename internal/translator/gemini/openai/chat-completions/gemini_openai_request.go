		...
		for i := 0; i < len(arr); i++ {
			...
			if (role == "system" || role == "developer") && len(arr) > 1 {
				...
				if tcs.IsArray() {
					...
					// Local: fall back to the call ID as the functionResponse name when the
					// tool-call name is unknown, so every call ID still gets a response part.
					responseParts := make([][]byte, 0, len(functionIDs))
					for _, functionID := range functionIDs {
						name := geminiOpenAIResponseFunctionName(tcID2Name[functionID], functionID)
