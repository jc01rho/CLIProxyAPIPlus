...
func (cfg *Config) SanitizeOAuthModelAlias() {
	...
	hasChannel := func(channel string) bool {
		for rawChannel := range cfg.OAuthModelAlias {
			if strings.EqualFold(strings.TrimSpace(rawChannel), channel) {
				return true
			}
		}
		return false
...
// SanitizeTokenThresholdRules normalizes token-range routing rules.
func (cfg *Config) SanitizeTokenThresholdRules() {
	if cfg == nil {
		return
	}
	out := make([]TokenThresholdRule, 0, len(cfg.Routing.TokenThresholdRules))
	for _, rule := range cfg.Routing.TokenThresholdRules {
