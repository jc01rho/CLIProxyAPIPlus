...
// FoldOpenCodeLegacyAPIKey makes api-key-entries the single source of truth
// when any nested keys are present.
func FoldOpenCodeLegacyAPIKey(entry *OpenCodeKey) {
	if entry == nil {
		return
	}
	legacy := strings.TrimSpace(entry.APIKey)
...
func (cfg *Config) SanitizeOpenCodeKeys() {
	...
	out := make([]OpenCodeKey, 0, len(cfg.OpenCodeKey))
	for _, entry := range cfg.OpenCodeKey {
		entry.APIKey = strings.TrimSpace(entry.APIKey)
		entry.BaseURL = strings.TrimSpace(entry.BaseURL)
		entry.ProxyURL = strings.TrimSpace(entry.ProxyURL)
		nested := make([]OpenAICompatibilityAPIKey, 0, len(entry.APIKeyEntries))
		...
		entry.APIKeyEntries = nested
		FoldOpenCodeLegacyAPIKey(&entry)
		entry.Prefix = normalizeModelPrefix(entry.Prefix)
		entry.BillingClass = normalizeBillingClass(entry.BillingClass)
		entry.Headers = NormalizeHeaders(entry.Headers)
		entry.ExcludedModels = NormalizeExcludedModels(entry.ExcludedModels)
