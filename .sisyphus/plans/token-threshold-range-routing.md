...
- [ ] F1. 요구사항 일치성 검토: `min-tokens` 기반 범위 규칙이 설정, API, 런타임에 모두 반영되었는지 확인한다.
- [ ] F2. 회귀 검증: 관련 Go 테스트와 빌드가 성공하는지 확인한다.
- [ ] F3. 위험 검토: 범위 해석, 경계값(1500/1501), 기존 규칙 하위호환성에 문제 없는지 검토한다.
- [ ] F4. 변경 범위 검토: 요청 범위를 벗어난 파일/기능 추가가 없는지 확인한다.
