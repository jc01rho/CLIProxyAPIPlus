...
- **Post-merge local-feature check (MANDATORY)**: upstream merges restructure `sdk/cliproxy/auth/conductor.go` (split into `conductor_execution.go`/`conductor_models.go`/etc.) and repeatedly drop local-only features (provider failover budget, threshold routing/billing class, alias-pool session pinning, route fallback, failure logging, quota-backoff constants, OAuth alias bypass, cooldown symbols). After every `git merge upstream/main`, run `go test ./sdk/cliproxy/auth/` — these tests encode the documented fork behavior (`README.md` ~lines 6-7). If they fail, restore the features from known-good commit `4ccff390`; do NOT edit or delete the tests. Full feature list and merge procedure live in the umbrella `~/git/cli-proxy/AGENTS.md` ("CLIProxyAPIPlus local-only 기능 보존").
