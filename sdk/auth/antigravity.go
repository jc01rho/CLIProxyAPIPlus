...
waitForCallback:
	...
	auth := BuildAntigravityAuth(&AntigravityTokenResponse{
	...
	auth.FileName = sanitizeAntigravityFileName(email)
...
func BuildAntigravityAuthURL(state, redirectURI string) string {
	...
	return authSvc.BuildAuthURL(state, redirectURI, nil)
}
...
func ExchangeAntigravityCode(ctx context.Context, code, redirectURI string, httpClient *http.Client) (*AntigravityTokenResponse, error) {
	...
	tokenResp, errExchange := authSvc.ExchangeCodeForTokens(ctx, code, redirectURI, "", nil)
	if errExchange != nil {
...
func FetchAntigravityUserInfo(ctx context.Context, accessToken string, httpClient *http.Client) (string, error) {
	// The headless API uses upstream's v2 endpoint; interactive login retains
	// the native client's v1 endpoint and operator-configured overrides.
	cfg := &config.Config{OAuthEndpointOverrides: map[string]config.OAuthEndpointConfig{
		"antigravity": {UserinfoURL: "https://www.googleapis.com/oauth2/v2/userinfo"},
	}}
	authSvc := antigravity.NewAntigravityAuth(cfg, httpClient)
