...
func TestHandlerProvidersForExecutionUsesRouterProvider(t *testing.T) {
	...
	providers, normalizedModel, errMsg := handler.providersForExecution(context.Background(), "ignored-by-router", "original-model", false, decision, modelExecutionOptions{})
	if errMsg != nil {
...
func TestHandlerProvidersForExecutionRejectsImageOnlyModelOnProviderRoute(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, _, errMsg := handler.providersForExecution(context.Background(), "ignored", tc.originalModel, false, tc.decision, modelExecutionOptions{})
