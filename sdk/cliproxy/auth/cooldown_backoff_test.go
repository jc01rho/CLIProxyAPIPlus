...
func TestApplyAuthFailureStateQuotaBackoffOncePerWindow(t *testing.T) {
	...
	secondRecover := secondFailureAt.Add(quotaBackoffBase * 2)
	...
	expectedRetryRecover := thirdFailureAt.Add(retryAfter)
	if !auth.Quota.NextRecoverAt.Equal(expectedRetryRecover) {
		t.Fatalf("expected retry hint window to close at %v, got %v", expectedRetryRecover, auth.Quota.NextRecoverAt)
	}
}
