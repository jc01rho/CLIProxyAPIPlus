...
// GetSelector is a local-compatible alias for Selector used by management handlers.
func (m *Manager) GetSelector() Selector {
	return m.Selector()
}

// ResetSelectorCycles resets weighted-robin cycles after config reloads that keep the same strategy.
		...
		for _, upstreamModel := range models {
			...
			if errExec != nil {
				...
				if isCountTokensEndpointNotFoundError(errExec, execReq.Model) {
					m.recordAvailabilityNeutralResult(attemptCtx, result)
				} else {
					m.MarkResult(attemptCtx, result)
				}
				if isRequestInvalidError(errExec) {
...
func (m *Manager) hasOpenAICompatAuthProvider(providers []string) bool {
	...
	defer m.mu.RUnlock()
	for _, auth := range m.auths {
		if auth == nil || !strings.EqualFold(strings.TrimSpace(auth.Provider), "openai-compatibility") {
			continue
		}
