...
func TestManager_MarkResult_CloudflareChallenge_On403(t *testing.T) {
	...
	if diff < 4*time.Minute || diff > 6*time.Minute {
		t.Fatalf("expected NextRetryAfter to be ~5 minutes (quotaBackoffBase), got %v", diff)
	}
...
func TestManager_RequestScopedErrorStopsCredentialFallbackWithoutSuspendingAuth(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "streaming bad request", stream: true, err: badRequestErr, wantFallback: true},
		...
		{name: "non-streaming plain bad request", err: plainBadRequestErr, wantFallback: true},
		{name: "streaming plain bad request", stream: true, err: plainBadRequestErr, wantFallback: true},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantFallback {
				if errExecute != nil {
					t.Fatalf("expected fallback to succeed, got %v", errExecute)
				}
				if len(calls) != 2 || calls[0] != badAuth.ID || calls[1] != goodAuth.ID {
