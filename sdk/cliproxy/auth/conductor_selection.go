...
// ResetSelectorCycles resets weighted-robin cycles after configuration reloads
// that retain the same selection strategy.
func (m *Manager) ResetSelectorCycles() {
	...
	selector := m.selector
	m.mu.RUnlock()
	if weighted, ok := UnwrapWeightedRobin(selector); ok {
		weighted.ResetCycles()
	}
}
...
// preserving the caller-visible route model.
...
func (m *Manager) executorLocked(provider string) (ProviderExecutor, bool) {
	...
	if !okExecutor && strings.HasPrefix(strings.ToLower(provider), "openai-compatible-") {
		executor, okExecutor = m.executors[strings.TrimPrefix(strings.ToLower(provider), "openai-compatible-")]
	}
	if !okExecutor {
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for _, candidate := range m.auths {
		...
		if !isWeightedRobin && canonicalSchedulingProvider(executorKeyFromAuth(candidate)) != targetKey {
			continue
		}
